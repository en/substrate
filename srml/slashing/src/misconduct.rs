...
pub struct Unresponsive {
	severities: rstd::vec::Vec<Fraction<u64>>,
	current_session: u32,
}
...
impl Misconduct for Unresponsive {
	...
	fn severity(&self) -> Fraction<Self::Severity> {
		...
		sum * Fraction::new(1_u64, Self::WINDOW_SIZE as u64)
	}
}
...
pub mod grandpa {
	...
	#[derive(Default)]
	pub struct Equivocation(rstd::vec::Vec<Fraction<u64>>);
	...
	impl Misconduct for Equivocation {
		...
		fn on_misconduct(&mut self, num_misbehaved: u64, num_validators: u64) {
			...
			let s = if denominator / numerator >= 1 {
				Fraction::new(1, 1)
			} else {
				Fraction::new(denominator, numerator)
			};

			self.0.resize(2, Fraction::zero());
			self.0.insert(0, s);
			self.0.pop();
		}
...
mod tests {
	...
	fn grandpa_invalid_vote_no_slash() {
		...
		invalid_vote.on_misconduct(0, 0);
